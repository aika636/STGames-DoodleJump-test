...
// Починка записей отдельным вызовом — его делает UI при открытии витрины и сразу
// сохраняет настройки. Раньше нормализация жила только внутри buySkin(): та мутировала
// settings и лишь потом могла вернуть ok: false, а UI сохранял только успешную ветку —
// починка мусора из settings.json жила до перезагрузки таверны, и так по кругу. Теперь
// «покупка» осталась чистой операцией, а починка — отдельной и всегда сохраняемой.
export function repairWallet(settings) {
    walletFor(settings);
    skinsFor(settings);
}
