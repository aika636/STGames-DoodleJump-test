...
export async function openShell({ gameId = null, args = {} } = {}) {
    ...
    root.tabIndex = -1;
    // Палитра ставится до монтирования экрана: игры читают переменные --djst-* при
    ...
    topbar.className = 'djst-topbar';

    const backBtn = document.createElement('button');
    backBtn.type = 'button';
    backBtn.className = 'djst-back';
    backBtn.textContent = '← Игры';
    backBtn.title = 'Вернуться к списку игр';

    const title = document.createElement('span');
    title.className = 'djst-title';

    topbar.append(backBtn, title);

    const screen = document.createElement('div');
    screen.className = 'djst-screen';
    ...
    const showHub = () => {
        ...
        const firstTile = hub.root.querySelector('.djst-tile');
        setTimeout(() => {
...
function showFallbackOverlay(content) {
    return new Promise((resolve) => {
        ...
        overlay.className = 'djst-overlay';

        const dialog = document.createElement('div');
        dialog.className = 'djst-dialog';
        dialog.setAttribute('role', 'dialog');
        dialog.setAttribute('aria-modal', 'true');
        dialog.setAttribute('aria-label', 'Дудл Джамп');

        const close = document.createElement('div');
        close.className = 'djst-close fa-solid fa-circle-xmark';
