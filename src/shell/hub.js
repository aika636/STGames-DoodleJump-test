...
export function createHub({ games, onPick, lastGameId }) {
    ...
    root.className = 'djst-hub';

    for (const game of games) {
        const tile = document.createElement('button');
        tile.type = 'button';
        tile.className = 'djst-tile';
        tile.dataset.gameId = game.id;
        if (game.id === lastGameId) tile.classList.add('djst-tile-last');

        const icon = document.createElement('i');
        icon.className = `fa-solid ${game.icon}`;

        const title = document.createElement('span');
        title.className = 'djst-tile-title';
        title.textContent = game.title;

        const tagline = document.createElement('span');
        tagline.className = 'djst-tile-tagline';
    ...
    const onClick = (event) => {
        const tile = event.target.closest?.('.djst-tile');
